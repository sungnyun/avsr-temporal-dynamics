...
class AVHubertPretrainingConfig(FairseqDataclass):
    ...
    noise_snr_sigma: float = field(default=5.0, metadata={'help': 'noise SNR std value'})
    noise_gaussian: bool = field(default=False, metadata={'help': 'use Gaussian noise'})
    noise_snr_thresh: int = field(default=100, metadata={'help': 'SNR threshold to consider the audio as noisy (<= threshold)'})
...
class AVHubertPretrainingTask(FairseqTask):
    ...
    def load_dataset(self, split: str, **kwargs) -> None:
        ...
        self.datasets[split] = AVHubertDataset(
            ...
            noise_snr=noise_snr,
            noise_snr_sigma=self.cfg.noise_snr_sigma,
            noise_gaussian=self.cfg.noise_gaussian,
            noise_snr_thresh=self.cfg.noise_snr_thresh,
            noise_num=noise_num
        )
