...
if DBG:
    from xmodal_av_hubert_dataset import XmodalAVHubertDataset
...
@dataclass
class XmodalAVHubertPretrainingConfig(FairseqDataclass):
    data: str = field(
    ...
    noise_gaussian: bool = field(default=False, metadata={'help': 'use Gaussian noise'})
    ...
    noise_with_clean: bool = field(default=False, metadata={"help": "return noisy wav data with clean wav data"})

@register_task("xmodal_av_hubert_pretraining", dataclass=XmodalAVHubertPretrainingConfig)
class XmodalAVHubertPretrainingTask(FairseqTask):

    cfg: XmodalAVHubertPretrainingConfig
    ...
    @classmethod
    def setup_task(
        cls, cfg: XmodalAVHubertPretrainingConfig, **kwargs
    ) -> "XmodalAVHubertPretrainingTask":
        if cfg.pdb:
            import pdb
            pdb.set_trace()
        return cls(cfg)
    ...
    def load_dataset(self, split: str, **kwargs) -> None:
        ...
        self.datasets[split] = XmodalAVHubertDataset(
            manifest,
            ...
            noise_snr=noise_snr,
            noise_num=noise_num,
            noise_with_clean=self.cfg.noise_with_clean
        )
