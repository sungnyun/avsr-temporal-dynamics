...
class XmodalAVHubertDataset(FairseqDataset):
    def __init__(
            self,
            manifest_path: str,
            sample_rate: float,
        ...
        self.noise_with_clean = noise_with_clean
    ...
    def load_feature(self, mix_name):
        ...
        if 'audio' in self.modalities:
            ...
            if self.noise_with_clean:
                clean_wav_data, clean_audio_feats = wav_data, None
            else:
                clean_wav_data, clean_audio_feats = None, None
            ...
            if clean_wav_data is not None:
                clean_audio_feats = logfbank(clean_wav_data, samplerate=sample_rate).astype(np.float32)
                clean_audio_feats = stacker(clean_audio_feats, self.stack_order_audio)
        else:
            audio_feats, clean_audio_feats = None, None
        ...
        if clean_audio_feats is not None and video_feats is not None:
            diff = len(clean_audio_feats) - len(video_feats)
            if diff < 0:
                clean_audio_feats = np.concatenate([clean_audio_feats, np.zeros([-diff, clean_audio_feats.shape[-1]], dtype=clean_audio_feats.dtype)])
            elif diff > 0:
                clean_audio_feats = clean_audio_feats[:-diff]
        return video_feats, audio_feats, clean_audio_feats
    ...
    def __getitem__(self, index):
        video_feats, audio_feats, clean_audio_feats = self.load_feature(self.names[index])
        ...
        clean_audio_feats = torch.from_numpy(clean_audio_feats.astype(np.float32)) if clean_audio_feats is not None else None
        if self.normalize and 'audio' in self.modalities:
            with torch.no_grad():
                audio_feats = F.layer_norm(audio_feats, audio_feats.shape[1:])
                if clean_audio_feats is not None:
                    clean_audio_feats = F.layer_norm(clean_audio_feats, clean_audio_feats.shape[1:])
        labels = self.get_labels(index)
        fid = self.names[index][1].split(':')[1]
        return {"id": index, 'fid': fid, "video_source": video_feats, 'audio_source': audio_feats, 'clean_audio_source': clean_audio_feats, "label_list": labels}
    ...
    def collater(self, samples):
        ...
        audio_source, clean_audio_source, video_source = [s["audio_source"] for s in samples], [s["clean_audio_source"] for s in samples], [s["video_source"] for s in samples]
        if audio_source[0] is None:
            audio_source = None
        if clean_audio_source[0] is None:
            clean_audio_source = None
        ...
        if clean_audio_source is not None:
            clean_audio_sizes = [len(s) for s in clean_audio_source]
        else:
            clean_audio_sizes = [len(s) for s in video_source]
        if self.pad_audio:
            audio_size = min(max(audio_sizes), self.max_sample_size)
            clean_audio_size = min(max(clean_audio_sizes), self.max_sample_size)
        else:
            audio_size = min(min(audio_sizes), self.max_sample_size)
            clean_audio_size = min(min(clean_audio_sizes), self.max_sample_size)
        ...
        if clean_audio_source is not None:
            collated_clean_audios, padding_mask, clean_audio_starts = self.collater_audio(clean_audio_source, clean_audio_size)
        else:
            collated_clean_audios, clean_audio_starts = None, None
        ...
        source = {"audio": collated_audios, "clean_audio": collated_clean_audios, "video": collated_videos}
