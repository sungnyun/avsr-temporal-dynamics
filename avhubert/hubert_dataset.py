...
class AVHubertDataset(FairseqDataset):
    def __init__(
        ...
        self.noise_snr_sigma = noise_snr_sigma
        self.noise_gaussian = noise_gaussian
        self.noise_snr_thresh = noise_snr_thresh
    ...
    def load_feature(self, mix_name):
        ...
        if 'audio' in self.modalities:
            ...
            if np.random.rand() < self.noise_prob:
                wav_data, noise_mask = self.add_noise(wav_data)
                # noise_mask = True
        ...
        return video_feats, audio_feats, noise_mask
    ...
    def select_noise(self):
        if self.noise_num > 1:
            size = np.random.randint(1, self.noise_num+1)
        else:
            size = self.noise_num
        rand_indexes = np.random.randint(0, len(self.noise_wav), size=size)
        noise_wav = []
    ...
    def add_noise(self, clean_wav):
        ...
        if type(self.noise_snr) == int or type(self.noise_snr) == float:
            ...
            if self.noise_gaussian:
                snr = np.random.normal(self.noise_snr, self.noise_snr_sigma)
        ...
        noise_mask = False
        if snr <= self.noise_snr_thresh:  # TODO: how much snr should be considered noisy?
            noise_mask = True
        return mixed, noise_mask

    def __getitem__(self, index):
        video_feats, audio_feats, noise_mask = self.load_feature(self.names[index])
        ...
        return {"id": index, 'fid': fid, "video_source": video_feats, 'audio_source': audio_feats, "label_list": labels, 'noise_mask': noise_mask}
    ...
    def collater(self, samples):
        ...
        collated_noise_mask = torch.BoolTensor([s["noise_mask"] for s in samples])
        ...
        source = {"audio": collated_audios, "video": collated_videos, "noise_mask": collated_noise_mask}
